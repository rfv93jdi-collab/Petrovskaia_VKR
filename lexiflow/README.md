...
Веб‑приложение для аудита юридических документов: извлечение ключевых фрагментов (пункты/сущности) и генерация описания рисков с рекомендациями. Фронтенд (Vite + React) и бэкенд (TypeScript) живут в одном проекте; API для анализа реализован в `server.ts`.

## Возможности

- Загрузка документов и запуск анализа через `/api/analyze-document`.
- Извлечение текста из PDF и изображений (OCR).
